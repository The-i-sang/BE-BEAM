  ...
  return (
    <div className="w-full bg-[#ffffff] pt-16">
      <div className="w-full max-w-[1400px] mx-auto flex flex-col justify-between items-center font-medium">
        ...
        <input
          className="w-full max-w-[760px] p-8 box-border mt-4 rounded-full border-[1px] border-solid border-[#79B1FF] outline-none text-[1.2rem] placeholder:text-[1.2rem] placeholder:text-[#79B1FF]"
          type="text"
          placeholder="툴킷을 검색하세요."
        />

        <div className="w-full mt-36 flex">
          <ul className="flex-1 pb-6 flex items-center text-5xl font-medium text-[#f5aa15]">
            {categories.map((cate) => {
              return (
                <li
                  key={cate.title}
                  onClick={() => handleCategoryClick(cate.title)}
                  className={`cursor-pointer xl:mr-3 md:mr-2 sm:mr-1 mr-1 2xl:px-10 xl:px-8 lg:px-7 md:px-4 sm:px-3 px-[0.6rem] xl:py-4 lg:py-3 md:py-3 sm:py-1 py-1 border-[1px] border-solid border-[#FDBA74] rounded-full ${
                    cate.isActive ? "bg-[#FDBA74] text-[white]" : ""
                  } hover:bg-[#FDBA74] hover:text-[white] transition-all duration-700 text-[#FDBA74] 2xl:text-[1rem] xl:text-[0.9rem] sm:text-[0.9rem] text-[0.9rem]`}
                >
                  {cate.title}
                </li>
              );
