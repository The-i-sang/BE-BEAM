...
export default function Menu({ path }) {
  ...
  return (
    <ul className="w-4/12 h-[80px] flex justify-between text-[#344054] text-[20px]">
      <li
        className={`${
          path === "/" ? "font-semibold after:block" : "after:hidden"
        } w-1/5 after:content-['*'] after:ml-0.5 after:text-red-500 after:hidden cursor-pointer hover:font-semibold transition-all duration-700 flex items-center`}
        onClick={() => {
          navigate("/");
        }}
      >
