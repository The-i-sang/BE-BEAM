  ...
  return (
    <div className="w-full py-16">
      <div className="w-11/12 mx-auto">
        ...
        <ul className="w-full mt-36 border-b-[1px] border-solid border-[#282828] flex items-center text-5xl font-medium text-[#f5aa15]">
          {categories.map((cate) => {
            return (
              <li
                key={cate.title}
                onClick={() => handleCategoryClick(cate.title)}
                className="cursor-pointer pb-8 border-b-[4px] border-[rgba(196,137,18,0)] border-solid mr-6 group"
              >
                <div className="flex flex-col items-center">
                  <div
                    className={`${
                      cate.isActive ? "bg-black" : "bg-transparent"
                    } w-[65px] h-[65px] rounded-full border-[1px] group-hover:bg-gradient-to-r group-hover:from-[#e5785d] group-hover:to-[#ffc66f] border-solid border-[#393939] flex justify-center items-center transition-all duration-700`}
                  >
                    {cate.title === "All" ? (
                      <p
                        className={`${
                          cate.isActive ? "text-white" : "text-[#393939]"
                        } text-[1.2rem] group-hover:text-white`}
                      >
                        ALL
                      </p>
                    ) : cate.title === "모집 중" ? (
                      <p
                        className={`${
                          cate.isActive ? "text-[#F5AA15]" : "text-[#F5AA15]"
                        } text-[1.2rem] group-hover:text-white`}
                      >
                        <BsSendCheck />
                      </p>
                      ...
                      <p
                        className={`${
                          cate.isActive ? "text-[#F5AA15]" : "[#F5AA15]"
                        } text-[1.2rem] `}
                      >
                        <BsSendSlash />
                      </p>
                  ...
                  <p
                    className={`${
                      cate.isActive ? "text-[#f58515] font-semibold" : ""
                    } mt-2 text-[1rem] font-medium [#F5AA15] group-hover:text-[#f58515] group-hover:font-semibold transition-all duration-700 group-hover:transition-all group-hover:duration-700`}
                  >
                    {cate.title}
                  </p>
                </div>
