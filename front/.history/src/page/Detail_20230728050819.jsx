...
export default function Detail() {
  ...
  const [recommendIndex, setRecommendIndex] = useState(null);

  // Filter the 'toolkits' array to get the selected 'toolkit' object
  const selectedToolkit = toolkits?.filter((t) => {
    return t.id !== params;
  });

  // Generate a random index between 0 and the length of the 'image' array
  const randomIndex = Math.floor(Math.random() * selectedToolkit?.length || 0);

  useEffect(() => {
    // Check if 'randomIndex' is a number and not NaN before setting 'recommendIndex'
    if (selectedToolkit && !isNaN(randomIndex)) {
      setRecommendIndex(randomIndex);
    }
  }, [selectedToolkit, randomIndex]);
