...
export default function Detail() {
  ...
  const recommendToolkit = toolkits.filter((toolkit) => toolkit.id !== params);
  const randomIndex = Math.floor(Math.random() * recommendToolkit.length);
      ...
      <div className="w-11/12 mx-auto py-10 flex justify-between">
        ...
        <div className="w-5/12 text-[#282828]">
          ...
          <div>
            {toolkits && (
              <>
                <p>{recommendToolkit[randomIndex].title}</p>
              </>
            )}
          </div>
        </div>
