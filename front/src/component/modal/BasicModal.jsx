...
export default function BasicModal({ setModal, wrapStyles, styles, children }) {
  return (
    <div
      className={`${wrapStyles} w-full h-screen bg-[rgba(0,0,0,0.4)] fixed top-0 left-0 transition-all duration-700`}
    >
      <div
        className={`${styles} w-11/12 max-w-[760px] p-6 box-border bg-white drop-shadow-xl border-[1px] border-solid border-[#ccc] rounded-2xl fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2`}
      >
        <Button
          icon={<GoX />}
